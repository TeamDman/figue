...
#[derive(Facet, Debug, PartialEq)]
struct ToArgsOptional {
    #[facet(args::named)]
    parallel: Option<Option<usize>>,
}

#[test]
fn optional_value_to_args_roundtrip() {
    let none = ToArgsOptional { parallel: None };
    assert_eq!(none.to_args().unwrap(), Vec::<std::ffi::OsString>::new());

    let bare = ToArgsOptional {
        parallel: Some(None),
    };
    assert_eq!(bare.to_args().unwrap(), vec!["--parallel"]);

    let valued = ToArgsOptional {
        parallel: Some(Some(12)),
    };
    assert_eq!(valued.to_args().unwrap(), vec!["--parallel=12"]);

    for original in [none, bare, valued] {
        ...
        let parsed: ToArgsOptional = figue::from_slice(&refs).unwrap();
